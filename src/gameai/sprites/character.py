...
class Character2D(CollidableObject2D):
    ...
    def __init__(self, settings: config.CharacterSettings):
        ...
        self.standing_on: Platform2D | None = None
        self.jumping = False
        self.inverted = False
    ...
    def move(self, keys: pygame.key.ScancodeWrapper):
        ...
        # TODO: use a key map so these are configurable
        ...
        # set dirty to redraw the sprite when moving
        # XXX: should characters be dirty sprites?
        if x != 0 or y != 0:
            self.dirty = 1
    ...
    def handle_collision(self, collision: Collision2D):
        ...
        self.standing_on = None
        if collision.bottom and isinstance(collision.bottom, Platform2D):
            self.standing_on = collision.bottom

    def update(self):
        self.image = self.image_inverted if self.inverted else self.image_normal
